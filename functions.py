...
def passes_and_key_passes(ax, selected_player_id, df):
    ...
    ax_text(
        0, -3,
        f'''<İsabetli Pas: {len(acc_pass)}/{len(df_passes)} ({accurate_pass_perc}%)> | <İsabetli Uzun Pas: {len(long_pass_acc)}/{len(long_pass)}>\n<Dikey Pas Oranı: {verticality_text}> | <Progresif Pas: {len(pro_pass)}> | <Kilit Pas: {len(df_key_passes)}>\n<Yaratılan Büyük Şans: {len(df_big_chances_created)}> | <Asist: {len(df_assists)}>''',
    ...
    if player_shots:
        ...
        for shot in player_shots:
            if shot['eventType'] == 'Goal':  # Gol olan şut
                ...
                pitch.scatter(shot['x'], shot['y'], ax=ax, c='None', s=round(shot['expectedGoals'], 2)*1000, edgecolors=goal_color, marker='football', alpha=0.8)
                ...
                pitch.scatter(shot['x'], shot['y'], ax=ax, c=shot_color, s=round(shot['expectedGoals'], 2)*1000, edgecolors='white', marker='D', alpha=0.8, lw=0.7, hatch='|||||')
                ...
                pitch.lines(shot['x'], shot['y'], shot['blockedX'], shot['blockedY'], ax=ax, color=to_rgba(shot_color, alpha=0.5), lw=1)
                pitch.scatter(shot['x'], shot['y'], ax=ax, c=shot_color, s=round(shot['expectedGoals'], 2)*1000, edgecolors='white', marker='o', alpha=0.8, lw=1, hatch='/////')
                ...
                pitch.scatter(shot['x'], shot['y'], ax=ax, c=shot_color, s=round(shot['expectedGoals'], 2)*1000, edgecolors='black', marker='x', alpha=0.8, lw=1)
            
            elif shot['eventType'] == 'Post':  # Direkten dönen şut
                shot_color = post_color
                pitch.lines(shot['x'], shot['y'], 105, shot['goalCrossedY'], ax=ax, color=to_rgba(shot_color, alpha=0.25), lw=1)
                pitch.scatter(shot['x'], shot['y'], ax=ax, c=shot_color, s=round(shot['expectedGoals'], 2)*1000, edgecolors='black', marker='+', alpha=0.8, lw=1, hatch='|')
            
            else:  # Bloklanan diğer şutlar
                shot_color = blocked_color
                pitch.lines(shot['x'], shot['y'], shot['blockedX'], shot['blockedY'], ax=ax, color=to_rgba(shot_color, alpha=0.5), lw=1)
                pitch.scatter(shot['x'], shot['y'], ax=ax, c=shot_color, s=round(shot['expectedGoals'], 2)*1000, edgecolors='black', marker='o', alpha=0.8, lw=1, hatch='/')
